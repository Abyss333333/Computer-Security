...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		if data['face_status'] == "enrolled":
			count +=1
		if data['voice_status'] == "enrolled":
			count +=1
		if data['fingerprint_status'] == "enrolled":
			count +=1
		if data['eye_status'] == "enrolled":
			count +=1
		if count >= 2:
			enrolled = 1
	...
	def sendStepup(self,
		...
		url =  'https://%s/v2/user_verifications' % \
			(hostname)
		params = {'user_uuid': self.userId, 'transaction_id': "HoppityBoppity", 'message': "Login request"}
		...
		verificationId = data["user_verification"]["uuid"]
	...
	def getStepupStatus(self):
		...
		data = result.json()
		return(data["user_verification"]["status"])
